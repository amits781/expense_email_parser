...
public class HdfcUpiParser implements BankEmailParser {
  ...
  public boolean canParse(EmailRequestBody email) {
    ...
    return (email.getFrom().contains("hdfcbank.net") || email.getFrom().contains("hdfcbank.com")
        || email.getFrom().contains("hdfcbank.bank.in")) && email.getSubject().contains("UPI txn");
  }
  ...
  public EmailResponseBody parse(EmailRequestBody email) {
    ...
    return EmailResponseBody.builder().tnxSource(formattedSource).tnxAmount(amount).tnxId(txnId)
        .tnxDate(DateUtils.getFormattedDate(email.getDate(), amount)).tnxDetails(details.trim())
        .build();
  }
